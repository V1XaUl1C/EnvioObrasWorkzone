...
lista_cx = [
    "CXEABT", "CXESBT", "CXGCAMT", "CXGCSBT", "CXGCSMT", 
    "CXICBT", "CXITBT", "CXSMT", "CXABT", "CXAMT", "CXSBT"
]
...
with col_link6:
    st.markdown("<p style='text-align:center; margin-bottom:5px; font-weight:bold; font-size:14px; color:#555;'>Aprobar Pre Liquidación</p>", unsafe_allow_html=True)
    ...
    if df_guia is not None:
        ...
        if col_btn1.button("🚀 Generar Plantilla Excel"):
            if not sed_seleccionada or not codigo_seleccionado or not tension_seleccionada or not contrato_seleccionado:
                ...
                st.download_button(
                    "📥 Descargar Plantilla", 
                    data=generar_excel(df_resultado), 
                    file_name=f"Plantilla_ODM_ING_{codigo_seleccionado}.xlsx"
                )
...
with tab2:
    ...
    with col_vas1:
        ...
        if archivo_vas is None:
            st.session_state.pop('datos_vas', None)
            st.session_state.pop('params_vas', None)
                    ...
                    if idx_tipo != -1 and idx_mat != -1 and idx_cant != -1 and idx_contador != -1:
                        if len(cols) > max(idx_tipo, idx_contador, idx_mat, idx_cant):
                            ...
                            if tipo in ["Materiales", "Servicios"]:
                                ...
                                if tipo == "Servicios" and len(matricula) >= 3: 
                                ...
                                if tipo == "Materiales": 
                                    bloque_materiales.append(row_data)
                                elif tipo == "Servicios":
    ...
    if 'datos_vas' in st.session_state:
        ...
        if agrupadores_con_vacios:
            ...
            for i, agp_vacio in enumerate(agrupadores_con_vacios):
                ...
                with cols_map[i % 3]:
                    ...
                    if seleccion != "(Dejar vacío)": 
                        mapeo_usuario[agp_vacio] = seleccion
        ...
        for i, row in enumerate(datos_vas):
            ...
            mat_final = ""
            if tipo == "Materiales": 
                mat_final = get_val(df_mat, 'Material SAP', matricula, 'Matricula EON TOBE')
            elif tipo == "Servicios": 
                mat_final = get_val(df_serv, 'Ser-Mat ASIS', matricula, 'Ser-Mat TOBE')
            elif tipo == "Espejo":
            ...
            if agrupador_busqueda in lista_cx:
                ...
                if not match_desc.empty: 
                    descripcion = match_desc.iloc[0][21]
        ...
        st.download_button(
            label="📥 Descargar Plantilla Completada (.xlsx)", 
            data=generar_excel(df_final),
            file_name=f"Plantilla_VAS_{params['pry']}.xlsx", 
...
with tab3:
    ...
    with col_ot1:
        ...
        if archivo_ot is None:
            st.session_state.pop('datos_ot', None)
            st.session_state.pop('params_ot', None)
            ...
            try:
                ...
                if not (col_agp and col_tipo and col_mat and col_cant):
                    ...
                    for _, row in df_ot_uploaded.iterrows():
                        ...
                        if not agrupador or agrupador == "nan": 
                            continue
    ...
    if 'datos_ot' in st.session_state:
        ...
        if agrupadores_con_vacios:
            ...
            for i, agp_vacio in enumerate(agrupadores_con_vacios):
                ...
                with cols_map[i % 3]:
                    ...
                    if seleccion != "(Dejar vacío)": 
                        mapeo_usuario_ot[agp_vacio] = seleccion
        ...
        st.download_button(
            label="📥 Descargar Plantilla OT Completada (.xlsx)", 
            data=generar_excel(df_final_ot),
            file_name=f"Plantilla_OT_{params['pry']}.xlsx", 
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
with tab4:
    ...
    archivos_versionar = st.file_uploader(
        "1. Sube los documentos (.docx, .xlsx, .pdf)", 
        type=["docx", "xlsx", "pdf"], 
        accept_multiple_files=True, 
        key=f"file_ver_{st.session_state['ver_key']}"
    )

    if not archivos_versionar:
        st.session_state.pop('zip_versionado', None)
